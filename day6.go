...
// The value of each position on the board, light on/off and brightness.
type light struct {
	state      bool
	brightness int
}
...
// Factory method for creating an initialized board struct.
func newBoard() board {
	b := make(board, 1000*1000)
	for y := 0; y < 1000; y++ {
		for x := 0; x < 1000; x++ {
			b[point{y, x}] = light{
				state:      false,
...
func (b board) lightsTurnedOn() int {
	count := 0
	for _, e := range b {
		if e.state {
			count++
...
// Returns the total brightness of the board.
func (b board) totalBrightness() int {
	brightness := 0
	for _, e := range b {
...
// Applies a given instruction to the board.
func (b board) apply(inst instruction) {
	for y := inst.fromy; y <= inst.toy; y++ {
		for x := inst.fromx; x <= inst.tox; x++ {
			// Fetch the value, cache the point.
			p := point{y, x}
			light := b[p]
			if inst.instruction == "turn on" {
				// Turn on and increase brightness by 1.
				light.state = true
				light.brightness++
			} else if inst.instruction == "turn off" {
				// Turn off light and decrease (if brightness is not 0).
				light.state = false
				if light.brightness > 0 {
					light.brightness--
				}
			} else if inst.instruction == "toggle" {
				// Toggle state and increase brightness by 2.
				light.state = !light.state
				light.brightness += 2

			} else {
				fmt.Fprint(os.Stderr, "Unknown instruction:", inst.instruction)
				os.Exit(1)
			}
...
// Wrapper around Atoi, os.Exit() if fails to parse.
func stringToInt(input string) int {
	result, err := strconv.Atoi(input)

	if err != nil {
		fmt.Fprintln(os.Stderr, "Unable to convert to int:", input)
		os.Exit(1)
	}
...
// Converts an instruction as a string into an "Instruction" type.
func parseInstruction(input string) instruction {
	if instRegex == nil {
		instRegex = regexp.MustCompile(
			`^(.+?)\s(\d+),(\d+)\sthrough\s(\d+),(\d+)$`)
	}
	rc := instRegex.FindStringSubmatch(input)

	if rc == nil {
		fmt.Fprintln(os.Stderr, "Unable to parse instruction:", input)
		os.Exit(1)
	}

	return instruction{
		instruction: rc[1],
		fromx:       stringToInt(rc[2]),
		fromy:       stringToInt(rc[3]),
		tox:         stringToInt(rc[4]),
		toy:         stringToInt(rc[5]),
	}
}
...
func main() {
	// Initialize board.
	b := newBoard()
	// Parse instructions and apply them to the board.
	before := time.Now()
	scanner := bufio.NewScanner(strings.NewReader(input))
	for scanner.Scan() {
		b.apply(parseInstruction(scanner.Text()))
	}

	// Print the result.
	fmt.Println("Lights turned on (part 1):", b.lightsTurnedOn())
	fmt.Println("Total brightness (part 2):", b.totalBrightness())
	fmt.Println("Total for both:", time.Now().Sub(before))
}
