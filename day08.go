...
// Takes the input, tries to unquote it and returns the length of the input
// minus the length of the unquoted string.
func inputMinusUnquoteLength(input string) int {
	unquote, err := strconv.Unquote(input)
	if err != nil {
		panic(err)
	}
	return len(input) - len(unquote)
}

// Takes the input, quotes it and return the length of the quoted string minus
// the length of the input.
func quotedMinusInputLength(input string) int {
	return len(strconv.Quote(input)) - len(input)
}

func main() {
	inputMinusQuotedResult := 0
	quotedMinusInputResult := 0

	// Iterate on the lines in the input, adding the difference in lengths to
	// Each sum.
	before := time.Now()
	scanner := bufio.NewScanner(strings.NewReader(input))
	for scanner.Scan() {
		input := scanner.Text()
		inputMinusQuotedResult += inputMinusUnquoteLength(input)
		quotedMinusInputResult += quotedMinusInputLength(input)
	}

	fmt.Println("Part 1:", inputMinusQuotedResult)
	fmt.Println("Part 2:", quotedMinusInputResult)
	fmt.Println("The whole thing took:", time.Now().Sub(before))
}
