...
func (sue auntSue) hasCompound(compound string, value int, part2 bool) bool {
	val, exists := sue[compound]

	// If the compound is not defined, it might be correct.
	if !exists {
		return true
	}

	// If we're doing part 2, make sure to check various compounds differently.
	if part2 {
	...
	// If the compound exists on the auntSue, validate that the values are
	// equal.
	if exists && val == value {
		return true
	}

	// The compound exists on the auntSue, but it's not valid against the given
	// compound.
	return false
}
...
func main() {
	// Parse the input.
	auntSues := make(map[string]auntSue)
	reParse := regexp.MustCompile(`(.*?): (.*?): (\d+), (.*?): (\d+), (.*?): (\d+)`)
	scanner := bufio.NewScanner(strings.NewReader(input))
	for scanner.Scan() {
		rc := reParse.FindStringSubmatch(scanner.Text())
		current := make(auntSue)
		current[rc[2]] = strToInt(rc[3])
		current[rc[4]] = strToInt(rc[5])
		current[rc[6]] = strToInt(rc[7])
		auntSues[rc[1]] = current
	}

	// Part 1
	before := time.Now()
	for key, sue := range auntSues {
		valid := true
		for compound, count := range compounds {
			if !sue.hasCompound(compound, count, false) {
				valid = false
				break
			}
		}
		...
		fmt.Println("Part 1 valid sue is:", key, sue)
	}
	...
	for key, sue := range auntSues {
		valid := true
		for compound, count := range compounds {
			if !sue.hasCompound(compound, count, true) {
				valid = false
				break
			}
		}
		...
		fmt.Println("Part 2 valid sue is:", key, sue)
	}
